...
| FR-31 | When sending a request, the user **explicitly selects which contact detail to share** — phone number **or** Telegram ID. Nothing is pre-selected. **Sharing one of them is required in order to send a request.** ⚠️ *Amended by CR-07 (2026-08-08): the "or nothing" option is struck and US-32 is retired. Mandatory sharing is not a default selection — the user still chooses which.* | Must | 1 |
...
| FR-38 | Rate limiting must cap join requests per user per time window, to prevent contact-harvesting and spam at scale. ⚠️ *Amended by CR-07 Q1 `B`: a **Round-1 courtesy limit of 5 requests per day** is now required, because it replaces the guard FR-31 removed. It is client-enforced and therefore **bypassable by clearing storage** — it stops accidental spam, not a determined harvester, and must never be described as a security control. **Real enforcement is server-side in Round 2** (US-34).* | Must | **1 (courtesy)** / 2 (enforced) |
