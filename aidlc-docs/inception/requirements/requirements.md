...
People have activities they are passionate about but lack a social circle that shares them. Existing options fail them: general social networks surface people you already know, and event platforms list commercial events rather than "I want to do this thing, come with me." There is no low-friction way to say _"I want to play D&D on Saturday, who's in?"_ and reach strangers nearby who care about exactly that.
...
| FR-31 | When sending a request, the user **explicitly selects which contact detail to share** — phone number **or** Telegram ID. Nothing is pre-selected. **Sharing one of them is required in order to send a request.** ⚠️ _Amended by CR-07 (2026-08-08): the "or nothing" option is struck and US-32 is retired. Mandatory sharing is not a default selection — the user still chooses which._                                                                                                               | Must     | 1                               |
...
| FR-38 | Rate limiting must cap join requests per user per time window, to prevent contact-harvesting and spam at scale. ⚠️ _Amended by CR-07 Q1 `B`: a **Round-1 courtesy limit of 5 requests per day** is now required, because it replaces the guard FR-31 removed. It is client-enforced and therefore **bypassable by clearing storage** — it stops accidental spam, not a determined harvester, and must never be described as a security control. **Real enforcement is server-side in Round 2** (US-34)._ | Must     | **1 (courtesy)** / 2 (enforced) |
...
| NFR-S6 | Authorization rules are expressed in shared, testable predicate functions (can this user see this address? rate this person? edit this activity?) so Round 2 can enforce the _same_ rules server-side. Client-side checks are UX, never the security boundary — this must be stated explicitly in the code. |
...
| **AR-02** | **Contact details sent immediately, with no approval gate, and — since CR-07 — compulsorily** (CQ4 `D`, CQ5; **re-accepted 2026-08-08 under CR-07**) | A person can post a fake activity to harvest phone numbers and Telegram IDs at scale. Once shared, a phone number cannot be revoked. ⚠️ **The risk INCREASED under CR-07**: every requester must now disclose, where previously a cautious user could take part sharing nothing. The population exposed to a fake activity is no longer self-selecting.                                                                                                                 | ⚠️ **Less mitigated than at first acceptance.** The requester still explicitly chooses _which_ detail, so each disclosure remains a consented act — but declining entirely is no longer possible, and consent to a required condition is weaker than consent to an optional one. | FR-32 mandatory in-UI disclosure (unchanged, and now load-bearing); ~~FR-31 "share nothing"~~ **retired by CR-07**; **FR-38 courtesy rate limit brought forward into Round 1** to partially replace it, bypassable and documented as such; FR-60/61 reporting, review console in R3. **Monitoring for harvesting patterns after launch moves from advisable to necessary** — the guard that made it optional is gone. |
...
| **AR-05** | **Blocking suppresses a blocked person's rating from the subject's aggregate** (U6 Q1 `A`, accepted 2026-08-09)                                      | ⚠️ **Blocking becomes a way to delete an unfavourable rating.** Someone rates you 1 star, you block them, your public average rises — repeatable at will. Ratings become effectively OPT-OUT, so the score measures who a person has not blocked rather than how they behave. This is the US-52 abuse guard defeated from the opposite end: US-52 stops a rating being manufactured by someone who did not attend; this lets one be un-manufactured by someone who did. | Accepted deliberately in exchange for **maximum separation** after a block — the safety case is that a person who blocks someone should not keep seeing their traces anywhere, and partial separation is confusing about what a block did.                                       | ⚠️ **Round 2 must recompute the aggregate server-side with blocks NOT applied**, so the public score stops being suppressible while the viewer-facing separation stays. Until then the vector is live and unmitigated. Monitor for accounts whose block count is disproportionate to their activity count — that is the signature.                                                                                    |
| **AR-06** | **A recurring series is not collapsed or down-weighted in the feed** (U5, accepted 2026-09-15) | A weekly venue publication materialises up to 8 near-identical `Activity` rows differing only in `startsAt`. They score almost identically under `core/rules/ranking` — same proximity, same category overlap — so **one venue's weekly night can occupy a long run of a viewer's feed**, crowding out one-off activities from people. | US-63's first criterion says occurrences appear *"as separate dated entries"* in as many words, so collapsing them into one card would contradict an approved story. Inventing a de-duplication or down-weighting rule the user has not seen is worse than naming the consequence. | Round 2 should revisit ranking with series awareness **once real venues exist and the crowding can be measured rather than guessed at**. The horizon (56 days, BR-U5-21) bounds the exposure: at most 8 rows per selected day-of-week per publication. |
