...
Raised on 2026-08-08 answering U4 Clarification Question 1 with `E` (a message thread on each join request), and withdrawn the same day: *"I've changed my mind / remove CR-06 for now."* CQ1 was then re-answered `D`, which goes in the opposite direction — see **CR-07**.
...
**Worth keeping in mind if it ever returns**: the strongest point in it was that in-app messaging *dissolves* the problem that a request sharing nothing is unactionable. That problem did not go away — CR-07 resolves it the other way, by removing the ability to share nothing at all.
