...
**Next action**: obtain approval for **U3 Code Generation (including CR-05)**, then begin **U4 Connections** — the largest unit and the one with the highest safety sensitivity (US-31 disclosure, US-52 rating eligibility).

**⚠️ U4 NOW INHERITS US-40 RATHER THAN BUILDING IT.** CR-05 shipped `RequestsInboxScreen` early — the read surface of the poster's inbox, carrying **the one INV-3 exception** (`sharedContact`). It is tested here (`tests/features/connections/requestsInbox.test.tsx`) at the repository boundary and on the wire, because the U4 verification that would have covered it will never happen. The rest of the loop — sending, the disclosure sheet, withdrawal, attendance, ratings — is still U4's.
