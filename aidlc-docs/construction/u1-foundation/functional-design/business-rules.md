...
→ *Property test, category: Idempotence.*
...
→ *Property test, category: Round-trip.* **Must include Esfand 29/30 boundaries and leap years in the generator.**
...
→ *Property test, category: Invariant.* Verified over the whole dataset — a hand-authored graph will otherwise drift.

**Rule BR-U1-24** — distance is **symmetric** and satisfies the **triangle inequality**: `d(a,c) ≤ d(a,b) + d(b,c)`.
→ *Property test, category: Invariant.*
...
→ *Property test, category: Invariant. Owned by U6, which builds the block filter; the contract is declared here.*
...
→ *Property test, category: Invariant. Owned by U3.*
...
→ *Property test, category: Invariant. Partly structural — `ProfileView` has no such fields — and partly behavioural for the one exception.*
...
→ *Property test, category: Round-trip.* **Generators must include optional fields both present and absent**, since the absent-vs-present distinction is exactly what INV-2 depends on and a naive JSON round-trip can turn an absent key into `undefined` or vice versa.
...
**Rule BR-U1-52** — refusals carry a **reason code**, never a bare boolean. US-52 requires the UI to explain *why* rating is unavailable; a boolean cannot.
