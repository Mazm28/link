...
Tests are *written* in Code Generation and *executed* in Build and Test; they were run here as well, and the results below are actual output rather than intent.
...
- `tests/app/shell.test.tsx` asserts the loading state is *reachable* — with an instant mock it would never render long enough to verify, and would break unnoticed until Round 2 introduced real latency.
